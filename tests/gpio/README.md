# RVX GPIO Module Tests

This directory contains the following tests for the RVX GPIO module:

- `testbench/` - Verilog RTL testbench compatible with Verilator and Vivado
- `unit_tests/` - Unit tests for the RVX HAL functionality related to the GPIO module

## Testbench

The testbench (`testbench/rvx_gpio_tb.v`) verifies the GPIO module's RTL implementation using either Verilator or Vivado.
...
```bash
cd tests/gpio/testbench/verilator
make run
```

A successful test run ends with the message: `Passed RTL testbench for the RVX GPIO module`.
...
3. Select `tests/gpio/testbench/vivado/create_test_project.tcl`
...
```bash
cd tests/gpio/testbench/vivado
make run
```

## HAL Unit Tests

The HAL unit tests verify that the RVX Hardware Abstraction Layer API works correctly with the GPIO module. These tests run on an **Arty A7-35T Development Board** using the test program `gpio_unit_tests.c`.
...
2. **Create the Vivado project**
   ...
   - Select `tests/gpio/unit_tests/create_test_project.tcl` and click **OK**
...
A successful test run ends with the message: `Passed all RVX HAL unit tests for the GPIO module`.