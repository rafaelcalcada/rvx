# RVX Timer Module Tests

This directory contains the following tests for the RVX Timer module:

- `testbench/` - Verilog RTL testbench compatible with Verilator and Vivado
- `unit_tests/` - Unit tests for the RVX HAL functionality related to the Timer module

## Testbench

The testbench (`testbench/rvx_timer_tb.v`) verifies the Timer module's RTL implementation using either Verilator or Vivado.
...
```bash
cd tests/timer/testbench/verilator
make run
```

A successful test run ends with the message: `Passed RTL testbench for the RVX Timer module`.
...
3. Select `tests/timer/testbench/vivado/create_test_project.tcl`
...
```bash
cd tests/timer/testbench/vivado
make run
```

## HAL Unit Tests

The HAL unit tests verify that the RVX Hardware Abstraction Layer API works correctly with the Timer module. These tests run on an **Arty A7-35T Development Board** using the test program `timer_unit_tests.c`.
...
2. **Create the Vivado project**
   ...
   - Select `tests/timer/unit_tests/create_test_project.tcl` and click **OK**
...
A successful test run ends with the message: `Passed all RVX HAL unit tests for the Timer module`.